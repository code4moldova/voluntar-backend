...
class TestRequests(ApiTestCase):
    def test_filter_by_beneficiary(self):
        ...
        response = self.get(
            url=f"/api/requests/filters/1/10?b_id={beneficiary.id}",
            user=operator
        )
            ...
            dict(
                ...
                list=[
                    {'_id': str(req2.id),
                     ...
                     'urgent': False,
                     'comments': None,
                     'has_symptoms': False, 'created_at': 'Sun, 02 Feb 2020 00:00:00 GMT',
                     'volunteer': {
                         '_id': str(req2.cluster.volunteer.id),
                         'first_name': req2.cluster.volunteer.first_name,
                         'last_name': req2.cluster.volunteer.last_name
                     }},
                    {'_id': str(req1.id), 'status': 'new', 'urgent': False, 'comments': None,
                     'has_symptoms': False, 'created_at': 'Wed, 01 Jan 2020 00:00:00 GMT'}
                ]
            ),
